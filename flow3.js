...
const gogogo = async (
  ...
  await page.evaluate(first_name => {
    return (document.getElementById("firstName").value = first_name);
  }, fname);
  await page.waitForSelector(LAST_NAME_INPUT);
  await page.click(LAST_NAME_INPUT);
  await page.evaluate(last_name => {
    return (document.getElementById("lastName").value = last_name);
  }, lname);

  await page.waitForSelector(ADDRESS_INPUT);
  await page.click(ADDRESS_INPUT);
  await page.evaluate(address => {
    return (document.getElementById("address1").value = address);
  }, my_address);

  await page.waitForSelector(CITY);
  await page.click(CITY);
  await page.evaluate(city => {
    return (document.getElementById("city").value = city);
  }, my_city);
  ...
  await page.evaluate(zip => {
    return (document.getElementById("postalCode").value = zip);
  }, my_zip);
  ...
  Promise.all(page.frames()).then(val =>
    val.forEach(async frame => {
      try {
        ...
        await frame.evaluate(cc => {
          return (document.getElementById("creditCardNumber").value += cc);
        }, my_cc);
        await frame.click(EXP_DATE);
        await frame.evaluate(exp_date => {
  ...
  if ((await page.$(CONTINUE_TO_ORDER_REVIEW)) !== null) {
    ...
    await page.evaluate(() =>
      Array.from(document.querySelectorAll(".placeOrderBtn"))[0].click()
    );
  } else {
